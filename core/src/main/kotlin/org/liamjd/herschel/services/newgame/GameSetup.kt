...
import org.liamjd.herschel.models.solarsystems.SolarSystemList
import java.io.File
...
class GameSetup {
	...
	fun loadPlayerNames(): List<Player> {
		...
		val playersFromJson = Json.decodeFromString(PlayerList.serializer(), playerListFile.readText())
	...
	fun loadHQList(): List<HQ> {
		...
		val hqsFromJson = Json.decodeFromString(HQList.serializer(), hqListFile.readText())
		return hqsFromJson.hqs
	}

	fun loadSolarSystem(): SolarSystem {
		val SOLSYSTEM_JSON = "assets/data/solarsystems/sol.json"
		val solFile = File(SOLSYSTEM_JSON)
		return Json.decodeFromString(SolarSystemList.serializer(),solFile.readText()).solarsystem.first()
	}
